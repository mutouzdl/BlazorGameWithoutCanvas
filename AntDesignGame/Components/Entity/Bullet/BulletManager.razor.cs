...
{
    private List<BulletConfig> _bulletConfigList = new();
    private Dictionary<BulletConfig, Bullet> _bulletMap = new();

    /* 子弹对象的uid和Bullet组件对应关系 */
    private Dictionary<int, BulletConfig> _bulletUIDDic = new Dictionary<int, BulletConfig>();
    ...
    {
        foreach (var bullet in _bulletMap.Values)
        {
            if (bullet != null)
            {
    ...
    public void Fire()
    {
        ...
        _bulletConfigList.Add(bulletConfig);
